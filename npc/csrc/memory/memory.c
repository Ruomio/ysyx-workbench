...
extern "C" void sdram_write(char ba, int row_addr, int col_addr, short int wstrb, int wdata) {
  ...
  if((wstrb & 0xffff) == 0xffff && ((addr & 0x3) == 0x0)) {
    ...
    if(len) {
      ...
      printf("trans mode!     sw_sdram_w: %d, sdram write, addr: 0x%x,  data: 0x%x,  wstrb:0x%x\n", sw_sdram_w, addr, wdata, wstrb);
    ...
    if(len) {
      paddr_write(addr, len, wdata);
      printf("normal mode!    sw_sdram_w: %d, sdram write, addr: 0x%x,  data: 0x%x,  wstrb:0x%x\n", sw_sdram_w, addr, wdata, wstrb);
    }
