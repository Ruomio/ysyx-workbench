...
void exec_once_npc(uint32_t pc) {
  ...
  while(!contextp->gotFinish()) {
    ...
    if(last_pc != g_get_pc()) {
      ...
      // idu_type = None;
      if((g_pc < CONFIG_MBASE) && (g_pc == 0x0f00007c) && 0) {
        u_npc_state.state = NPC_ABORT;
        u_npc_state.pc = pc;
        u_npc_state.ret = true;
        return;
...
uint32_t g_get_dnpc() {
  ...
  return g_dnpc;
#elif defined (ysyx_24080020_NPC)
  if(top->rootp->ysyx_24080020_NPC__DOT__u_cpu__DOT__u_reg__DOT__is_dnpc_wb) {
    g_dnpc = top->rootp->ysyx_24080020_NPC__DOT__u_cpu__DOT__u_reg__DOT__dnpc_wb;
  }
  else
    g_dnpc = g_pc + 4;
  return g_dnpc;
