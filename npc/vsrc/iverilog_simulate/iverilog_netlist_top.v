...
module sim_top;
    ...
    always @(posedge clock) begin
        ...
        else if(u_cpu.exu_mem_valid  && u_cpu.mem_exu_ready ) begin
            pc_delay_cnt <= 'h0;
            // $display("%h", u_cpu.pc_exu );
        end
        else begin
            pc_delay_cnt <= pc_delay_cnt + 1'b1;
            if(pc_delay_cnt > 32'h200) begin
                $display("EXU lag at pc: 0x%h", u_cpu.pc_exu );
    ...
    always @(posedge clock) begin
        ...
        // else if(u_cpu.is_ebreak_lsu ) begin
        else if(u_cpu.idu_exu_valid  && u_cpu.exu_idu_ready  &&
                    u_cpu.is_ebreak_idu) begin
            $display("ebreak inst!");
            $finish;
        end
    end
