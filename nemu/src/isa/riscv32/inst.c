...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, R(rd) = s->snpc; s->dnpc = s->pc + imm; ftrace_update(s->pc, s->dnpc, BITS(s->isa.inst.val, 19, 15), BITS(s->isa.inst.val, 11, 7)));
  INSTPAT("??????? ????? ????? ??? ????? 11001 11", jalr   , I, R(rd) = s->snpc; s->dnpc = (src1 + imm)&~1u; ftrace_update(s->pc, s->dnpc, BITS(s->isa.inst.val, 19, 15), BITS(s->isa.inst.val, 11, 7)));
