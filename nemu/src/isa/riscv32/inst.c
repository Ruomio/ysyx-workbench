...
static vaddr_t *get_csr_reg(word_t csr) {
  // printf("csr idx = 0x%x\n", csr);
  if(csr == MEPC_ADDR) {return &cpu.csrs[mepc];}
...
static void mret(Decode *s) {
  ...
  /* todo:
   * achive privilege mode

   * privilege_mode = tmp_mstatus & (3 << 11);

   * 改变 MPP 为 U/M mode. bit[12:11]
   * tmp_mstatus |= (3 << 11); // M mode
   * tmp_mstatus &= ~(3 << 11); // U mode
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 100 ????? 11000 11", blt    , B, ((int)src1 < (int)src2) ? (s->dnpc = s->pc + imm) : 0);
  INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge    , B, ((int)src1 >= (int)src2) ? (s->dnpc = s->pc + imm) : 0);
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai   , I, R(rd) = (SEXT(src1, 32) >> BITS(imm, 5, 0)));

  INSTPAT("0000000 ????? ????? 000 ????? 01100 11", add    , R, R(rd) = (int)src1 + (int)src2);
  INSTPAT("0100000 ????? ????? 000 ????? 01100 11", sub    , R, R(rd) = (int)src1 - (int)src2);
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 01100 11", sra    , R, R(rd) = (SEXT(src1, 32) >> BITS(src2, 5, 0)));
