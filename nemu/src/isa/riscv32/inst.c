...
#define immJ() do { *imm = (SEXT(BITS(i, 31, 31), 1) << 20) | (SEXT(BITS(i, 30, 21), 10) << 0) | (SEXT(BITS(i, 20, 20), 1) << 10) | (SEXT(BITS(i, 19, 12), 8) << 12) ; } while(0)
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", j      , J, R(rd) = s->pc + 4; s->dnpc += imm);
