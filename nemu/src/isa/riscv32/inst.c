...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 001 ????? 11100 11", csrrw  , I, R(rd) = CSR(imm()); CSR(imm()) = src1; printf("csrrw imm():0x%x", (unsigned int)imm()); assert(0) );
  INSTPAT("??????? ????? ????? 010 ????? 11100 11", csrrs  , I, printf("inst imm() = 0x%x\n", (unsigned int)imm());R(rd) = CSR(imm()); CSR(imm()) = CSR(imm()) | src1; IFDEF(CONFIG_DiFFTEST ,difftest_skip_ref());/*assert(0)*/ );
