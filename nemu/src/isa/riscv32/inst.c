...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 001 ????? 11100 11", csrrw  , I, R(rd) = CSR(imm()); CSR(imm()) = src1; IFDEF(CONFIG_DIFFTEST ,difftest_skip_ref());/*printf("csrrw imm():0x%x\n", (unsigned int)imm()); assert(0)*/ );
  INSTPAT("??????? ????? ????? 010 ????? 11100 11", csrrs  , I, R(rd) = CSR(imm()); CSR(imm()) = CSR(imm()) | src1; IFDEF(CONFIG_DIFFTEST ,difftest_skip_ref());/*printf("inst imm() = 0x%x\n", (unsigned int)imm()); assert(0)*/ );
