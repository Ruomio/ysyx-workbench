...
CacheSim::CacheSim(std::string path) {
  ...
  cache_tag = new std::vector<std::vector<uint32_t>>(
      cachenum, std::vector<uint32_t>(cacheway, 0));
  cache_valid = new std::vector<std::vector<bool>>(
      cachenum, std::vector<bool>(cacheway, false));
  cache_data = new std::vector<std::vector<std::vector<uint32_t>>>(
      cachenum, std::vector<std::vector<uint32_t>>(
                    cacheway, std::vector<uint32_t>(cachesize / 4, 0)));
}
...
void CacheSim::print_results() {
  ...
  assert(inst_cnt > 0);
  if (!mode) {
    std::cout << "ICache_Hit: " << cache_hit << " Percentage:" << hit_rate * 100
              << "%" << std::endl;
    std::cout << "ICache_Miss: " << cache_miss
              << " Percentage:" << miss_rate * 100 << "%" << std::endl;
    std::cout << "ICache: " << "AMAT: "
              << HIT_CYCLES * hit_rate + MISS_CYCLES * miss_rate
              << " TMT: " << MISS_CYCLES * cache_miss << std::endl;
  } else {
    std::cout << "DCache_Hit: " << cache_hit << " Percentage:" << hit_rate * 100
              << "%" << std::endl;
    std::cout << "DCache_Miss: " << cache_miss
              << " Percentage:" << miss_rate * 100 << "%" << std::endl;
    std::cout << "DCache: " << "AMAT: "
              << HIT_CYCLES * hit_rate + MISS_CYCLES * miss_rate
              << " TMT: " << MISS_CYCLES * cache_miss << std::endl;
  }
}
  ...
  for (uint32_t i = 0; i < cacheway; i++) {
    if ((*(this->cache_tag))[index][i] == tag &&
      ...
      if ((*cache_data)[index][i][offset] == address) {
        ...
        printf("error hit, not just only cache miss! should be:0x%x, but get: "
               "0x%x\n",
...
void CacheSim::run_simulation() {
  ...
  if (!inst_file.is_open()) {
    std::cerr << "Error opening instruction file: " << inst_file_path
              << std::endl;
    return;
  }
  ...
  while (std::getline(inst_file, line)) {
    ...
    if (is_cachehit(address)) {
      ...
      // Simulate storing data in the cache (for simplicity, just store the
      // address)
